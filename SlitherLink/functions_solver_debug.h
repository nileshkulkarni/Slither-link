...
void Loop::HorDark (int i, int j) {
     ...
     if (HorLine[i][j]==1) if (cons) {
        ...
        HorLine[i][j]=0;
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Darkened horizontal ("<<i<<","<<j<<").";
        
        if (GridCer[i][j]==GridVal[i][j]) NumLeft--;
        if (GridCer[i+1][j]==GridVal[i+1][j]) NumLeft--;
        
        if (VertCer[i][j]==3 || VertCer[i][j-1]==3) {cons=false; cout<<"\nInconsistency while darkening.";}
        else if (GridVal[i][j]!=-1) {if (GridCer[i][j]>GridVal[i][j]) {cons=false; cout<<"\nInconsistency while darkening.";}}
        else if (GridVal[i+1][j]!=-1) {if (GridCer[i+1][j]>GridVal[i+1][j]) {cons=false; cout<<"\nInconsistency while darkening.";}}
        
        cout<<VertCer[i][j-1]<<", "<<VertCer[i][j]<<"; "<<LoopNumber[i][j-1]<<", "<<LoopNumber[i][j];
        if (VertCer[i][j-1]==1) {
           if (VertCer[i][j]==1) {
              maxSegNum++;
              LoopNumber[i][j]=LoopNumber[i][j-1]=maxSegNum;
              cout<<"\nSet LoopNumber of ("<<i<<","<<j<<"). as "<<LoopNumber[i][j];
              cout<<"\nSet LoopNumber of ("<<i<<","<<j-1<<"). as "<<LoopNumber[i][j-1];
           }
        
           else Convert(i,j-1,1);
        }
     
        else if (LoopNumber[i][j]==LoopNumber[i][j-1]) {if (Solved()!=0) {cons=false; cout<<"\nInconsistency in loop numbers.";}}
        else Convert (i,j,3);
...
void Loop::VerDark (int i, int j) {
     ...
     if (VerLine[i][j]==1) if (cons){
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Darkened vertical ("<<i<<","<<j<<").";
        
        if (GridCer[i][j]==GridVal[i][j]) NumLeft--;
        if (GridCer[i][j+1]==GridVal[i][j+1]) NumLeft--;
        
        if (VertCer[i][j]==3 || VertCer[i-1][j]==3) {cons=false; cout<<"\nInconsistency while darkening.";}
        else if (GridVal[i][j]!=-1) {if (GridCer[i][j]>GridVal[i][j]) {cons=false; cout<<"\nInconsistency while darkening.";}}
        else if (GridVal[i][j+1]!=-1) {if (GridCer[i][j+1]>GridVal[i][j+1]) {cons=false; cout<<"\nInconsistency while darkening.";}}
        
        cout<<VertCer[i-1][j]<<", "<<VertCer[i][j]<<"; "<<LoopNumber[i-1][j]<<", "<<LoopNumber[i][j];
        if (VertCer[i-1][j]==1) {
           if (VertCer[i][j]==1) {
              maxSegNum++;
              LoopNumber[i][j]=LoopNumber[i-1][j]=maxSegNum;
              cout<<"\nSet LoopNumber of ("<<i<<","<<j<<"). as "<<LoopNumber[i][j];
              cout<<"\nSet LoopNumber of ("<<i-1<<","<<j<<"). as "<<LoopNumber[i-1][j];
           }
        
           else Convert(i-1,j,2);
        }
     
        else if (LoopNumber[i][j]==LoopNumber[i-1][j]) {if (Solved()!=0) {cons=false; cout<<"\nInconsistency in loop numbers.";}}
        else Convert (i,j,0);
...
void Loop::HorCross (int i, int j) {
     ...
     if (HorLine[i][j]==1) if (cons) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Crossed horizontal ("<<i<<","<<j<<").";
        if (GridImp[i][j]>4-GridVal[i][j] || GridImp[i+1][j]>4-GridVal[i+1][j]) {cons=false; cout<<"\nInconsistency while crossing.";}
...
void Loop::VerCross (int i, int j) {
     ...
     if (VerLine[i][j]==1) if (cons) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Crossed vertical ("<<i<<","<<j<<").";
        if (GridImp[i][j]>4-GridVal[i][j] || GridImp[i][j+1]>4-GridVal[i][j+1]) {cons=false; cout<<"\nInconsistency while crossing.";}
     ...
     if (cons) if (VertCer[i][j]+VertImp[i][j]!=4) {
        if (Coupled[i][j]==-1) {
           ...
           SetMax(i,j,3-v);
             ...
             cout<<endl;
             for (u=1; u<indent; u++) cout<<" ";
             cout<<"Coupled ("<<i<<","<<j<<","<<v<<"), which was already coupled the other way.";
             sHorCross(i, j);
             sVerCross(i, j);
             sHorCross(i, j+1);
             sVerCross(i+1, j);
...
void Loop::sHorDark(int i, int j) {
     ...
     if (HorLine[i][j]==-1) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Unsuccessful attempt to darken horizontal ("<<i<<","<<j<<")";
     }
     indent--;
}
...
void Loop::sVerDark(int i, int j) {
     ...
     if (VerLine[i][j]==-1) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Unsuccessful attempt to darken vertical ("<<i<<","<<j<<")";
     }
     indent--;
}
...
void Loop::sHorCross(int i, int j) {
     ...
     if (HorLine[i][j]==0) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Unsuccessful attempt to cross horizontal ("<<i<<","<<j<<")";
     }
     indent--;
}
...
void Loop::sVerCross(int i, int j) {
     ...
     if (VerLine[i][j]==0) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Unsuccessful attempt to cross vertical ("<<i<<","<<j<<")";
     }
     indent--;
}
...
void Loop::Convert (int i, int j, int dir) {
     ...
     if (cons) {
        ...
        cout<<endl;
        for (u=1; u<indent; u++) cout<<" ";
        cout<<"Set LoopNumber of ("<<i<<","<<j<<"). as "<<LoopNumber[i][j];
