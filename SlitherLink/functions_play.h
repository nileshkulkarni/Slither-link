     ...
     if (HorLine[i][j]==1) if (cons) {
        ...
        if (diffic>0) {
           ...
           if (GridCer[i+1][j]==GridVal[i+1][j]-1) if (HorLine[i+1][j]==1) {
              if (VerLine[i+1][j-1]==1) SetMax(i+1,j-1,1);
              if (VerLine[i+1][j]==1) SetMax(i+1,j,0);
           }
           
           for (u=0;u<4;u++) {
               if (Max1[i][j-hadd(u)][u]) sVerCross(i+vadd(u),j-hadd(u));
               if (VerLine[i+vadd(u)][j-hadd(u)]==-1) {
                  SetMin(i, j-hadd(u), (u+2)%4);
                  SetMax(i, j-hadd(u), (u+2)%4);
               }
           }
        }
     }
     ...
     if (VerLine[i][j]==1) if (cons){
        ...
        if (diffic>0) {
           if (GridCer[i][j]==GridVal[i][j]-1) if (VerLine[i][j-1]==1) {
              if (HorLine[i-1][j]==1) SetMax(i-1,j-1,2);
              if (HorLine[i][j]==1) SetMax(i,j-1,1);
           }
           
           if (GridCer[i][j+1]==GridVal[i][j+1]-1) if (VerLine[i][j+1]==1)  {
              if (HorLine[i-1][j+1]==1) SetMax(i-1,j+1,3);
              if (HorLine[i][j+1]==1) SetMax(i,j+1,0);
           }
           
           for (u=0;u<4;u++) {
               if (Max1[i-vadd(u)][j][u]) sHorCross(i-vadd(u),j+hadd(u));
               if (HorLine[i-vadd(u)][j+hadd(u)]==-1) {
                  SetMin(i-vadd(u), j, (u+2)%4);
                  SetMax(i-vadd(u), j, (u+2)%4);
               }
           }
        }
     }
     ...
     if (HorLine[i][j]==1) if (cons) {
               ...
               VerCross(i,j-u);
                 ...
                 if (VertCer[i][j-u]==1) {
                    ...
                    VerDark(i,j-u);
                      ...
                      if (VerLine[i][j-u]==-1) Couple (i,j-u,u);
                      if (VerLine[i+1][j-u]==-1) Couple (i,j-u,1-u);
                 }
            }
        }
        ...
        if (diffic>0) {
           ...
           if (GridVal[i+1][j]!=-1) if (GridImp[i+1][j]==3-GridVal[i+1][j]) if (HorLine[i+1][j]==1) {
              if (VerLine[i+1][j-1]==1) SetMin(i+1,j-1,1);
              if (VerLine[i+1][j]==1) SetMin(i+1,j,0);
           }
     ...
     if (VerLine[i][j]==1) if (cons) {
               ...
               HorCross(i-u,j);
                 ...
                 if (VertCer[i-u][j]==1) {
                    ...
                    HorDark(i-u,j);
                      ...
                      if (HorLine[i-u][j]==-1) Couple (i-u,j,u);
                      if (HorLine[i-u][j+1]==-1) Couple (i-u,j,1-u);
                 }
            }
        }
        ...
        if (diffic>0) {
           if (GridVal[i][j]!=-1) if (GridImp[i][j]==3-GridVal[i][j]) if (VerLine[i][j-1]==1) {
              if (HorLine[i-1][j]==1) SetMin(i-1,j-1,2);
              if (HorLine[i][j]==1) SetMin(i,j-1,1);
           }
           
           if (GridVal[i][j+1]!=-1) if (GridImp[i][j+1]==3-GridVal[i][j+1]) if (VerLine[i][j+1]==1) {
              if (HorLine[i-1][j+1]==1) SetMin(i-1,j+1,3);
              if (HorLine[i][j+1]==1) SetMin(i,j+1,0);
           }
